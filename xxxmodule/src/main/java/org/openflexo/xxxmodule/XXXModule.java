...
public class XXXModule extends FlexoModule<XXXModule> {
	...
	protected FlexoController createControllerForModule() {
		return new XXXController(this);
	}

	@Override
	public XXX getModule() {
		return getApplicationContext().getModuleLoader().getModule(XXX.class);
	}

	public XXXController getXXXController() {
		return (XXXController) getFlexoController();
	}

	@Override
	public XXXPreferences getPreferences() {
		return (XXXPreferences) super.getPreferences();
	}
	...
	public boolean close() {
		if (getApplicationContext().getResourceManager().getUnsavedResources().size() == 0) {
			...
			if (getXXXController().reviewModifiedResources()) {
				return super.close();
			}
			else {
				return false;
